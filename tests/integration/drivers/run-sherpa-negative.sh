#!/usr/bin/env bash
# run-sherpa-negative.sh — Pivot/Pause (negative results) integration test
#
# Two-phase test:
#   Phase 1: Generate P5 artifacts to EL Draft (same as standard P5)
#   Phase 2: Resume with negative experiment results that hit falsification thresholds
#
# Verifies: EL outcome is pause, no DPRD generated, ER updated with pause status.
#
# Usage: bash run-sherpa-negative.sh
...
RUN_DIR="$OUTPUT_DIR/sherpa-negative-$TIMESTAMP"
PROJECT_DIR="$RUN_DIR/aieos-aicr"
PHASE1_FIXTURE="$INTEGRATION_DIR/fixtures/aicr-exploratory"
PHASE2_FIXTURE="$INTEGRATION_DIR/fixtures/aicr-negative"
...
if [[ ! -f "$SKILL_FILE" ]]; then
  ...
  print_summary "Sherpa Pivot/Pause (Negative)"
  exit 1
fi
...
# ─── Phase 2: Resume with negative results ───────────────────────────────────

log_step "Phase 2: Resume with negative experiment results"
...
## AUTOMATED TEST MODE — SESSION RESUMPTION (NEGATIVE RESULTS)

You are running in automated test mode. This is PHASE 2 of a two-phase test.

**Context:** Phase 1 has already run. The project directory at $PROJECT_DIR contains P5 artifacts from the AICR initiative. EL is Draft, all prior artifacts are Frozen.

**Important test mode rules:**
- Do NOT ask questions — use the scripted responses below
- Do NOT ask \"Ready?\" — just proceed
- FIRST: Scan the project directory for existing artifacts and the ER
- Identify current position: EL is Draft, waiting for experiment results
- The user is returning with NEGATIVE experiment results
- Fill in the EL with the results — all falsification thresholds were hit
- The EL outcome should be PAUSE (not proceed)
- Do NOT generate a DPRD — the hypothesis is falsified
- Update the ER with pause status
- Freeze the EL with the pause outcome
- Use this project directory: $PROJECT_DIR
- The AIEOS framework is at: $AIEOS_ROOT
...
Discover existing state, resume the session, process negative experiment results, set EL outcome to pause, freeze EL, do NOT generate DPRD. Write transcript to $RUN_DIR/phase2-transcript.md including:
1. What existing artifacts you discovered
2. The EL update with negative results
3. The pause decision and rationale
4. Confirmation that no DPRD was generated
5. Final ER state with pause status"
...
# EL should be Frozen with pause outcome
if ls "$PROJECT_DIR/docs/sdlc/"*el*.md &>/dev/null; then
  EL_FILE=$(ls "$PROJECT_DIR/docs/sdlc/"*el*.md 2>/dev/null | head -1)
  if grep -q "Frozen" "$EL_FILE"; then
    log_pass "EL is Frozen"
  else
    log_fail "EL should be Frozen after Phase 2"
  fi
  if grep -qi "pause\|stop\|discontinue\|falsif" "$EL_FILE"; then
    log_pass "EL contains pause/falsification outcome"
  else
    log_fail "EL should indicate pause outcome"
  fi
fi

# DPRD should NOT exist
if ls "$PROJECT_DIR/docs/sdlc/"*dprd*.md &>/dev/null; then
  log_fail "DPRD was generated — should NOT exist for pause outcome"
else
  log_pass "No DPRD generated (correct for pause outcome)"
fi
