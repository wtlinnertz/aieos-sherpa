...
**Decision check:** After the journal entry, check whether this artifact represents or influenced a key decision. Decision triggers: scope boundary changed from upstream, architectural trade-off made, cross-cutting kit adopted/declined, release strategy chosen, assumption pivoted. If a decision was made, append a decision entry to the ER's Key decisions subsection for the current layer using the format from ER spec §Decision Register Governance: `DEC-{INITIATIVE}-{NNN} | {type} | {description} | {artifact_id} | {date}`. Decision IDs are sequential across all layers.
...
- **Update the artifact's own Document Control section** — set `Status: FROZEN` (the canonical **uppercase** freeze vocabulary: `DRAFT | VALIDATED | FREEZE_PENDING | FROZEN`, defined once in `aieos-schema/schema/document-control.yaml`, FR-018). Also set `Last Validation: PASS` — validation outcome is a **separate, orthogonal field** from freeze status (`Last Validation: PASS | FAIL`), never folded into `Status`. Add `Frozen By` and `Frozen Date`. The artifact file itself must reflect its frozen state, not just the ER.
